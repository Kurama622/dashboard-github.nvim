...
return function(ns)
	...
	api.nvim_set_hl(ns, "DashboardGreen", { bg = mix(exgreen, bg, 80), fg = exgreen })
	...
	api.nvim_set_hl(ns, "DashboardBlue", { bg = mix(exblue, bg, 80), fg = exblue })
	...
	api.nvim_set_hl(ns, "DashboardYellow", { bg = mix(exyellow, bg, 80), fg = exyellow })
	...
	if transparent then
		...
		api.nvim_set_hl(ns, "DashboardGrey", { bg = lighten(bg, 6 * x), fg = lighten(commentfg, 10 * x) })
	end
	vim.api.nvim_set_hl(ns, "DashboardArt", { link = "DashboardRed0" })
	vim.api.nvim_set_hl(ns, "DashboardKey", { link = "DashboardGreen1", bold = true })
