...
-- The function is written by https://github.com/nvzone/typr
...
M.activity_heatmap = function(data)
  ...
  local week = utils.get_week_of_past_year(os.date(
    "*t",
    os.time({
      year = time.year,
      month = time.month,
  ...
  for i = 1, state.months_to_show do
    local idx = (i + month_offset - 1) % 12 == 0 and 12
  ...
  local hrline = M.separator(
    "─",
    squares_len * 2 + (state.months_to_show - 1 + 5),
    "exlightgrey"
  )
  ...
  for i = 1, state.months_to_show do
    ...
    for n = 1, days_in_months[month_i] do
      ...
      local hl = "Dashboard"
        .. monthshl[month_i]
        .. utils.get_activity_hl(activity)
...
local function calculate_positions()
  ...
  for _, line in ipairs(state.art) do
    art_display_width =
      math.max(art_display_width, vim.fn.strdisplaywidth(line))
  end

  local max_right_display_width = 0

  for _, line in ipairs(state.shortcuts) do
    max_right_display_width = math.max(
      max_right_display_width,
      vim.fn.strdisplaywidth(line.desc) + vim.fn.strdisplaywidth(line.key) + 4
    )
  end

  state.layout.datetime_str = get_datetime()

  max_right_display_width = math.max(
    max_right_display_width,
    vim.fn.strdisplaywidth(state.layout.datetime_str)
  )
  local total_display_width = art_display_width
    + state.layout.gap
    + max_right_display_width

  local start_pos =
    math.max(1, math.floor((screen_width - total_display_width) / 2))

  return {
    art_left_margin = start_pos,
    right_section_left = start_pos + art_display_width + state.layout.gap,
  }
end
...
local function render()
  ...
  for _ = 1, state.layout.top_offset do
    table.insert(lines, "")
  end
  ...
  for i, art_line in ipairs(state.art) do
    local line_idx = state.layout.top_offset + i
  ...
  if date_line_idx <= #lines then
    ...
    local needed_spaces = math.max(
      state.layout.date_gap,
      pos.right_section_left - 1 - #current_line
    )
    local new_line = current_line
      .. string.rep(" ", needed_spaces)
      .. state.layout.datetime_str
    lines[date_line_idx] = new_line

    local date_byte_start = #current_line + needed_spaces
    local date_byte_end = date_byte_start + #state.layout.datetime_str

    table.insert(highlights_to_apply, {
      line = date_line_idx - 1,
      col_start = date_byte_start,
      col_end = date_byte_end,
      hl_group = state.highlights.date,
    })
  end

  local cursor = {}

  local shortcuts = state.shortcuts
  for i, shortcut in ipairs(shortcuts) do
    local row_idx = shortcuts_start_idx + i - 1
    if row_idx <= #lines then
      local shortcut_text =
        string.format("[%s]  %s", shortcut.key, shortcut.desc)

      local current_line = lines[row_idx] or ""
      local needed_spaces =
        math.max(state.layout.gap, pos.right_section_left - 1 - #current_line)
      local new_line = current_line
        .. string.rep(" ", needed_spaces)
        .. shortcut_text
      if i == 1 then
      ...
      table.insert(highlights_to_apply, {
        ...
        hl_group = state.highlights.desc,
      })
    end
  end
