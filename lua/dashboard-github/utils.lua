...
function M.get_n_months_ago(n)
	...
	local t = os.time({ year = year, month = month, day = day, hour = now.hour, min = now.min, sec = now.sec })
...
function M.opeator_date(t, delta)
...
function M.render_heatmap(buf, lines)
	...
	for row, segments in ipairs(lines) do
		...
		for _, h in ipairs(highlights) do
			vim.hl.range(buf, state.ns, h.group, { row - 1, h.start_col }, { row - 1, h.end_col })
		end
	end
